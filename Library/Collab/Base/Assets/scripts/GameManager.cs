		...
		foreach (Item t in itemList.ToArray()) {
			if (t.itemName == "boule" && !t.Equals (ball)) {
				...
				if (a * a > 0.001) {
					...
					if ((dist + diffSpeed * Time.deltaTime).magnitude < (ball.diametre / 2) + (otherBall.diametre / 2)) {
				...
				if (t.itemName == "mur") {
					...
					if (((nextPos.x + ball.diametre / 2 - wall.transform.position.x < max1) &&
					    (nextPos.x + ball.diametre / 2 - wall.transform.position.x > min1) &&
					    (nextPos.z + ball.diametre / 2 - wall.transform.position.z < max2) &&
					    (nextPos.z + ball.diametre / 2 - wall.transform.position.z > min2)) ||
					    ((nextPos.x - ball.diametre / 2 - wall.transform.position.x < max1) &&
					    (nextPos.x - ball.diametre / 2 - wall.transform.position.x > min1) &&
					    (nextPos.z + ball.diametre / 2 - wall.transform.position.z < max2) &&
					    (nextPos.z + ball.diametre / 2 - wall.transform.position.z > min2)) ||
					    ((nextPos.x + ball.diametre / 2 - wall.transform.position.x < max1) &&
